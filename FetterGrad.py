...
class FetterGrad():
    ...
    def _project_conflicting(self, grads, has_grads, shapes=None):
        ...
        fetter_grad, num_task = copy.deepcopy(grads), len(grads)
        for g_i in fetter_grad:
            random.shuffle(grads)
            for g_j in grads:
                g_i_g_j = torch.dist(g_i, g_j, p=2)  # Euclidean distance
                g_i_g_j = 1 / (1 + g_i_g_j)
                if g_i_g_j < 0.5:
                    g_i += (g_i_g_j) * g_j
        ...
        if self._reduction:
            merged_grad[shared] = torch.stack([g[shared]
                                           for g in fetter_grad]).mean(dim=0)
        elif self._reduction == 'sum':
            merged_grad[shared] = torch.stack([g[shared]
                                           for g in fetter_grad]).sum(dim=0)
